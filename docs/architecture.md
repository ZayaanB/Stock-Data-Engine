...
Historical network captures take a second input route. The PCAP reader validates record lengths, unwraps Ethernet and VLAN tags, validates IPv4 and UDP boundaries, and passes only the UDP payload to `MoldDecoder`. MoldUDP64 messages are validated transactionally before dispatch. `SequenceTracker` follows the ten-byte session and expected 64-bit sequence, reporting gaps and rewinds while recognizing heartbeat and end-of-session packets.
...
The optional threaded file route places a 65,536-item `SpscRing<Message>` between the reader and book. Monotonic producer/consumer counters occupy separate 64-byte-aligned cache lines. Release stores publish items and acquire loads observe them; the mutation engine remains single-threaded and lock-free.
